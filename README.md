...
Install Codex from **Settings → AI & Integrations → Codex / ChatGPT desktop connection**. **Install / repair desktop connection** safely merges the packaged MCP server into the shared Codex `config.toml` using the exact active installation path. It preserves existing settings and other MCP servers, creates a timestamped backup, and does not require the Codex CLI to be installed or available on `PATH`. **Check connection** verifies both the saved desktop registration and the DeepScribe `status` tool. Completely restart Codex and start a new task afterwards so the tools are loaded.
